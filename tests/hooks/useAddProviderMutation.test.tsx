...
describe("useAddProviderMutation", () => {
  ...
  it("restores and returns the fixed Codex official seed", async () => {
    const seedProvider: Provider = {
      id: "codex-official",
    ...
    apiMocks.getAll.mockResolvedValueOnce({ "codex-official": seedProvider });
    const { wrapper } = createWrapper();
    ...
    expect(apiMocks.ensureCodexOfficialProvider).toHaveBeenCalledOnce();
    expect(apiMocks.add).not.toHaveBeenCalled();
    expect(persisted).toEqual(seedProvider);
  });
});
