...
"#
    ...
    let config_text = std::fs::read_to_string(agent_switch_lib::get_codex_config_path())
...
"#;
    ...
    let auth_after_switch: serde_json::Value =
        read_json_file(&agent_switch_lib::get_codex_auth_path()).expect("read auth after switch");
    assert_eq!(
    ...
    let auth_after_takeover: serde_json::Value =
        read_json_file(&agent_switch_lib::get_codex_auth_path()).expect("read auth after takeover");
    assert_eq!(
    ...
    let restored_auth: serde_json::Value =
        read_json_file(&agent_switch_lib::get_codex_auth_path()).expect("read restored auth");
    assert_eq!(
...
fn reapply_codex_official_live_resyncs_mcp_servers() {
    ...
    let live = std::fs::read_to_string(agent_switch_lib::get_codex_config_path())
    ...
    let reapplied = agent_switch_lib::reapply_current_codex_official_live(&state)
...
fn sync_all_enabled_reports_broken_app_but_projects_the_rest() {
    ...
    let err = agent_switch_lib::McpService::sync_all_enabled(&state)
...
fn provider_service_switch_codex_official_accounts_write_auth_json() {
    ...
    let auth_b: serde_json::Value =
        read_json_file(&agent_switch_lib::get_codex_auth_path()).expect("read auth B");
    assert_eq!(
        auth_b
            .pointer("/tokens/access_token")
            .and_then(|v| v.as_str()),
    ...
    let auth_a: serde_json::Value =
        read_json_file(&agent_switch_lib::get_codex_auth_path()).expect("read auth A");
    assert_eq!(
        auth_a
            .pointer("/tokens/access_token")
            .and_then(|v| v.as_str()),
...
fn switch_packycode_gemini_updates_security_selected_type() {
    ...
    // Gemini security settings are written to ~/.gemini/settings.json, not ~/.agent-switch/settings.json
...
/// 下一个供应商；供应商专属字段、密钥与 AGS 注入产物绝不进片段；
...
fn switch_codex_syncs_shared_keys_from_live_into_common_config() {
    ...
    // disable_response_storage + AGS 注入产物 + MCP 同步投影
...
"#;
    ...
    let live_after = std::fs::read_to_string(agent_switch_lib::get_codex_config_path())
        .expect("read config.toml after switch");
    assert!(
