...
mod tests {
    ...
    fn extract_codex_common_config_strips_provider_fields_and_injected_artifacts() {
        ...
        // web_search = "disabled" 是 AGS 对黑名单网关注入的哨兵；
...
"#;
        ...
        assert!(
            ...
            "should strip the AGS web_search disabled sentinel, got: {extracted}"
        );
        // 真正可共享的键保留
        assert!(
...
"#
    ...
    async fn update_current_claude_desktop_provider_syncs_profile_when_proxy_takeover_is_active() {
        ...
        assert_eq!(
            ...
            json!("http://127.0.0.1:42567/claude-desktop"),
...
impl ProviderService {
    ...
    /// 已剥离全部供应商专属与 AGS 注入内容：`model` / `model_provider` /
    ...
    fn extract_codex_common_config(settings: &Value) -> Result<String, AppError> {
        ...
        // - web_search 只剥 AGS 注入的 "disabled" 哨兵；用户手设的其它值
        //   属于可共享偏好，保留。
        if root
