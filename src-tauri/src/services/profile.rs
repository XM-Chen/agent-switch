...
use crate::database::{CcAggregateConfig, Profile};
...
pub struct ProfilePayload {
    ...
    /// AGS Claude 聚合模式配置；非 Claude scope 保持 None。
    pub aggregate_config: PerApp<Option<CcAggregateConfig>>,
}
    ...
    pub fn merge_scope_from(&mut self, other: &ProfilePayload, scope: ProfileScope) {
        for app in scope.apps() {
            ...
            if let (Some(dst), Some(src)) = (
                self.aggregate_config.get_mut(app),
                other.aggregate_config.get(app),
            ) {
                *dst = src.clone();
            }
        }
    ...
    pub fn scope_captured(&self, scope: ProfileScope) -> bool {
        scope.apps().iter().any(|app| {
            self.providers.get(app).is_some_and(|s| s.is_some())
                ...
                || self.aggregate_config.get(app).is_some_and(|s| s.is_some())
        })
    }
}
...
impl ProfileService {
    ...
    pub fn snapshot_current(
        ...
        for app in scope.apps().iter() {
            ...
            if matches!(app, AppType::Claude) {
                payload.aggregate_config.claude =
                    Some(state.db.get_cc_aggregate_config(app.as_str())?);
            }
        }
        Ok(payload)
    }
    ...
    pub fn apply(
        ...
        for app in scope.apps().iter() {
            ...
            // 6. AGS Claude 聚合配置（旧 payload / 非 Claude scope 为 None，不动）。
            if matches!(app, AppType::Claude) {
                if let Some(Some(config)) = payload.aggregate_config.get(app) {
                    if let Err(e) = state.db.set_cc_aggregate_config(app_str, config) {
                        warnings.push(format!("[{app_str}] restore aggregate config failed: {e}"));
                    }
                }
            }
        }
...
mod tests {
    ...
    fn test_payload_serde_roundtrip() {
        let payload = ProfilePayload {
            ...
            aggregate_config: PerApp {
                claude: Some(CcAggregateConfig::default()),
                claude_desktop: None,
                codex: None,
            },
        };
    ...
    fn test_payload_tolerates_missing_fields() {
        ...
        assert_eq!(back.aggregate_config, PerApp::default());
