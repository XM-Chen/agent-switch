...
mod tests {
    ...
    fn response(content_type: Option<&str>) -> ProxyResponse {
    ...
    #[test]
    fn json_content_type_accepts_suffixes() {
        assert!(response(Some("application/json; charset=utf-8")).is_json());
        assert!(response(Some("application/problem+json")).is_json());
        assert!(!response(Some("text/event-stream")).is_json());
        assert!(!response(None).is_json());
    }
}
