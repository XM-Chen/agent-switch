...
async fn handle_claude_transform(
    ...
    // 转换失败也必须记录上游已消耗的 Responses tokens。
...
/// Converts an Anthropic Messages upstream response back to Responses for Codex.
async fn handle_codex_anthropic_to_responses_transform(
    ...
    // Buffer explicit JSON so 2xx error envelopes and gateways that ignore
    // stream:true can still be converted faithfully.
    ...
    let anthropic_response: Value = match serde_json::from_slice(&body_bytes) {
        Ok(value) => value,
        Err(_) if body_looks_like_sse(&body_str) => {
            log::warn!("[Codex] 上游返回未标记的 Anthropic SSE 响应，按消息聚合兜底");
            transform_codex_anthropic::anthropic_sse_to_message_value(&body_str).map_err(|e| {
                log::error!("[Codex] Anthropic SSE 聚合失败: {e}");
                e
            })?
        }
        Err(e) => {
            log::error!("[Codex] 解析 Anthropic 上游响应失败: {e}, body: {body_str}");
            return Err(upstream_body_parse_error(
    ...
    let responses_response =
        ...
        .map_err(|e| {
            log::error!("[Codex] Anthropic -> Responses 响应转换失败: {e}");
            e
        })?;
    ...
    let usage_collector = if usage_logging_enabled(state) {
        ...
        Some(SseUsageCollector::new(
            ...
            move |events, first_token_ms| {
                ...
                if !usage.has_billable_tokens() {
                    log::debug!("[Codex] Anthropic 流式响应 usage 全 0 或缺失，跳过消费记录");
                    return;
                }
                let model = usage
                    .model
...
fn codex_proxy_error_json(
    ...
    let message = if upstream_status == Some(413) {
        ...
        let status_fragment = upstream_status
            ...
            .unwrap_or_default();
        format!(
            "Agent Switch local proxy failed while handling Codex endpoint {endpoint}. Provider: {provider_name}; model: {request_model}{status_fragment}; cause: {cause}"
        )
    };
...
data: {\"type\":\"response.output_item.done\",\"item\":{\"type\":\"message\"}}\n\n";
    ...
    fn codex_proxy_forward_error_includes_context_and_cause() {
        ...
        assert!(message.contains("Agent Switch local proxy failed"));
