...
///   且不一定等于用户目录，可能导致 `.agent-switch/agent-switch.db` 路径变化，从而“看起来像数据丢失”。
///
/// ## 测试隔离
///
/// 为了让 Windows CI/本地测试能稳定隔离真实用户数据，可通过 `AGENT_SWITCH_TEST_HOME`
...
pub fn get_app_config_dir() -> PathBuf {
    ...
    // v3.10.3 可能在 `HOME/.agent-switch/` 下创建/使用了数据库。
