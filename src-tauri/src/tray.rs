...
fn handle_auto_click(app: &tauri::AppHandle, app_type: &AppType) -> Result<(), AppError> {
    if let Some(app_state) = app.try_state::<AppState>() {
        ...
        // Auto 模式依赖网关路由，接管走 proxy route_mode。
        log::info!("[Tray] Auto 模式：对 {app_type_str} 执行接管");
        if let Err(e) = futures::executor::block_on(proxy_service.set_takeover_for_app(
            app_type_str,
            true,
...
pub fn handle_tray_menu_event(app: &tauri::AppHandle, event_id: &str) {
    ...
    match event_id {
        ...
        "open_website" => {
            if let Err(e) = app
                .opener()
                .open_url("https://github.com/XM-Chen/agent-switch", None::<String>)
            {
