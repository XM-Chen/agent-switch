...
impl TestHomeGuard {
    fn new() -> Self {
        ...
        let original_test_home = env::var_os("AGENT_SWITCH_TEST_HOME");
...
fn test_parse_invalid_scheme() {
    let url = "httpx://v1/import?resource=provider&app=claude&name=Test";
...
#[test]
fn test_parse_paste_compat_ccswitch_scheme() {
    // Paste-compat: legacy ccswitch:// still parses; system only registers agentswitch.
    let url = "ccswitch://v1/import?resource=provider&app=claude&name=Legacy%20Paste&homepage=https%3A%2F%2Fexample.com&endpoint=https%3A%2F%2Fapi.example.com&apiKey=sk-legacy";

    let request = parse_deeplink_url(url).unwrap();
    assert_eq!(request.version, "v1");
    assert_eq!(request.resource, "provider");
    assert_eq!(request.app, Some("claude".to_string()));
    assert_eq!(request.name, Some("Legacy Paste".to_string()));
}
...
fn test_import_prompt_allows_space_in_base64_content() {
    ...
    let url = "agentswitch://v1/import?resource=prompt&app=codex&name=PromptPlus&content=Pj4+";
    let request = parse_deeplink_url(url).unwrap();
...
fn test_parse_prompt_deeplink() {
    ...
    let url = format!(
        "agentswitch://v1/import?resource=prompt&app=claude&name=test&content={}&description=desc&enabled=true",
        content_b64
    );
...
fn test_parse_mcp_deeplink() {
    ...
    let url = format!(
        "agentswitch://v1/import?resource=mcp&apps=claude,codex&config={}&enabled=true",
        config_b64
    );
...
#[test]
#[serial_test::serial]
fn test_mcp_import_is_database_only() {
    ...
    let config = BASE64_STANDARD
        .encode(r#"{"mcpServers":{"db-only-import":{"type":"stdio","command":"echo"}}}"#);
    let request = parse_deeplink_url(&format!(
        "agentswitch://v1/import?resource=mcp&apps=codex&config={config}"
    ))
    .expect("parse request");
    let imported = import_mcp_from_deeplink(&state, request).expect("import into database");
...
fn test_parse_skill_deeplink() {
    let url = "agentswitch://v1/import?resource=skill&repo=owner/repo&directory=skills&branch=dev";
    let request = parse_deeplink_url(url).unwrap();
